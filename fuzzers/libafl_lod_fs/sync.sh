...
##
# Host-side staging script for the libafl_lod_fs fuzzer.
#
...
# fuzzers/libafl_lod_fs/sources/ gets COPYed into the image, where fetch.sh then
...
if [ ! -f "$LOD_SRC/magma-forkserver/Cargo.toml" ]; then
    echo "sync.sh: '$LOD_SRC' doesn't look like lod-sketch" \
         "(missing magma-forkserver/Cargo.toml)." >&2
    exit 1
fi
